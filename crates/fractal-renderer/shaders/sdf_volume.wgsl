...
fn sample_volume(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (global_id.x >= vertex_count.x || global_id.y >= vertex_count.y || global_id.z >= vertex_count.z) {
        return;
    }

    // Compute normalized coordinates [0, 1] across the grid vertices
    let t = vec3<f32>(
        f32(global_id.x) / f32(volume_params.grid_size.x),
        f32(global_id.y) / f32(volume_params.grid_size.y),
        f32(global_id.z) / f32(volume_params.grid_size.z),
    );
    ...
    // Store at linear index: x + y * vertex_count.x + z * vertex_count.x * vertex_count.y
